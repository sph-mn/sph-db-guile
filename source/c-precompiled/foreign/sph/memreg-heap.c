...
similar to sph/memreg.c but uses a specialised heap allocated array for the
memory register that can be passed between functions usage: memreg_register_t
allocations; if(!memreg_heap_allocate(4, &allocations)) { return(1); }
 ...
 */
#define memreg_heap_declare(variable_name) \
  i_array_declare(variable_name, memreg_register_t)
/** true on success, false on failure (failed memory allocation) */
#define memreg_heap_allocate(register_size, register_address) \
  i_array_allocate_memreg_register_t(register_size, register_address)
